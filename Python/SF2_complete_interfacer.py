...
def main():
    ...
    rx_thread = threading.Thread(target=thread_eth_tx, args=())
    rx_thread.start()
    while True:
        plot_data = thread_eth_rx()
        plt.plot(plot_data)
        plt.show()

    rx_thread.join()

    #with concfut.ThreadPoolExecutor(max_workers=2) as executor:
    #    fut_eth_rx = executor.submit(thread_eth_rx)
    #    fut_eth_tx = executor.submit(thread_eth_tx)
    #
    #    print(fut_eth_rx)
    #    print(fut_eth_tx)
...
def thread_eth_tx():
    ...
    logging.info('samples generated')
    plt.plot(samples)
...
def thread_eth_rx():
    ...
    while True:
        er = 0
        logging.info('hello, I\'m listening')
        try:
        ...
        if(er == 0):
            ...
            if data:# and addr[0] == '100.100.100.200':
                logging.info('\tdata bytes: ' + str(len(data)))
                return list(data)
